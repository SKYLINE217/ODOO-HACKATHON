    ...
    <div className="min-h-screen bg-[var(--bg-base)] flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden font-sans">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md z-10 text-center">
        ...
        <p className="text-xs font-mono text-[var(--text-secondary)] uppercase tracking-wider mt-1">
          Join VendorBridge procurement network
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div className="bg-[var(--bg-surface)] border-t-[5px] border-t-[var(--accent)] border border-[var(--border-default)] py-8 px-4 shadow-xl rounded-xl sm:px-10 relative">
          ...
          {message && (
            <div className={`mb-6 p-4 rounded text-xs font-mono font-bold border-l-4 ${
              message.type === 'success' 
                ? 'bg-emerald-500/10 border-l-emerald-600 border-emerald-500/20 text-emerald-600' 
                : 'bg-rose-500/10 border-l-rose-600 border-rose-500/20 text-rose-600'
            }`}>
              {message.text}
            </div>
          )}
            ...
            <div>
              ...
              <div className="mt-1.5 relative rounded-md shadow-sm">
                ...
                <input
                  ...
                  className="block w-full pl-9 pr-3 py-2 bg-[var(--bg-subtle)] border border-[var(--border-default)] rounded text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] text-sm transition-all font-mono"
                />
              </div>
            </div>
          ...
          <div className="mt-5 text-center border-t border-[var(--border-default)] pt-4">
            <span className="text-xs text-[var(--text-secondary)]">
              Already have an account?{' '}
              <Link href="/login" className="text-[var(--accent)] hover:text-[var(--accent-hover)] font-bold transition-colors font-mono">
                SIGN IN
              </Link>
            </span>
          </div>
          ...
          <div className="mt-5 border-t border-[var(--border-default)] pt-5">
            <div className="bg-[var(--bg-subtle)] border-l-4 border-l-[var(--accent)] border border-[var(--border-default)] rounded p-3 flex gap-2.5 items-start">
              <ShieldCheck size={18} className="text-[var(--accent)] shrink-0 mt-0.5" />
              <p className="text-[10px] font-mono text-[var(--text-secondary)] leading-normal">
                <strong>FAST-TRACK ENROLLMENT:</strong> Email confirmations are bypassed automatically. Your login request will process immediately using local credential registries.
              </p>
            </div>
          </div>
        </div>
